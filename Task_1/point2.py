...
def generate_signal():
    ...
    if signal_type.get() == "Sine":
        ...
        # Testing Sin_output
        SignalSamplesAreEqual('SinOutput.txt', samples, indices)
    else:
        indices = amplitude * np.cos(2 * np.pi * analog_frequency * samples + phase_shift_theta)
        # Testing Cos_output
        SignalSamplesAreEqual('CosOutput.txt', samples, indices)
...
generate_button.grid(row=7, column=0, columnspan=3, pady=5)

# Matplotlib plot setup
fig, ax = plt.subplots(nrows=1, ncols=2, figsize=(15, 6))
canvas = FigureCanvasTkAgg(fig, master=t)
canvas_widget = canvas.get_tk_widget()
canvas_widget.grid(row=8, column=0, columnspan=2, pady=6)
