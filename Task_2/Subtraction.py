...
class SignalProcessingApp:
    def __init__(self, root):
        ...
        self.load_button = tk.Button(root, text="Load and Subtrac Signals",font=("Helvetica", 15), bg="blue", fg="white", command=self.load_signals)   
    ...
    def plot_signals(self):
        ...
        if len(continuous_signals) >= 2:
            result =  abs(continuous_signals[0] - continuous_signals[1])
        ...
        if len(continuous_signals) >= 2:
            ...
            self.ax[1].clear()
            self.ax[1].set_title('Signals Subtraction')
            
        
        self.ax[2].clear()
        self.ax[2].plot(time, result, label='Result', color='green')
        self.ax[2].legend()
        self.ax[2].set_title('Subtraction Result Visualization',color='red')
